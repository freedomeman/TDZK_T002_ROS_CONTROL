    ...
    double motor_zero_offset)
{
    ...
    } else if (motor_type == "ENCOS") {
        return std::make_shared<EncosMotorDriver>(
            motor_id, interface_type, interface,
            static_cast<ENCOS_Motor_Model>(motor_model), motor_zero_offset);
    } else if (motor_type == "STW") {
        return std::make_shared<StwMotorDriver>(
            motor_id, interface_type, interface, master_id_offset,
            static_cast<StwMotorModel>(motor_model), motor_zero_offset);
    } else {
