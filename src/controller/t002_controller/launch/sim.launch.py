...
def generate_launch_description():
    ...
    gazebo_resource_path = SetEnvironmentVariable(
        ...
        value=[share_root, os.pathsep,
               EnvironmentVariable("GZ_SIM_RESOURCE_PATH", default_value="")],
    )

    # 通过 xacro 参数把 controller config 传入 URDF
    ...
    spawn_entity = Node(
        package="ros_gz_sim",
        executable="create",
        arguments=["-topic", "robot_description", "-name", "T002_Robot",
                    "-z", "0.3"],
        output="screen",
    )
    ...
    # Controller spawners (不需要 ros2_control_node, Gazebo 提供)
    jb_spawner = Node(
    ...
    t002_spawner = Node(
        package="controller_manager",
        executable="spawner",
        arguments=["t002_controller", "-c", "/controller_manager",
                    "--controller-manager-timeout", "20"],
        output="screen",
    )
    ...
    return LaunchDescription([
        DeclareLaunchArgument("xacro_file",
            default_value=PathJoinSubstitution(
                [pkg_description, "xacro", "T002_gazebo_des.urdf.xacro"])),
        DeclareLaunchArgument("controller_config",
            default_value=PathJoinSubstitution(
                [pkg_controller, "config", "controller.yaml"])),
