...
class DmImuNode(Node):
    def __init__(self):
        ...
        #debug
        # Yaw 数据记录
        self._yaw_log_file = None          # 文件句柄，惰性打开
        self._yaw_log_path = '/home/tuf/Doc/TDZK_T002_ROS_CONTROL/imu_yaw_log.csv'   # 可自定义路径

        self._yaw_start_time = None   # 用于 yaw 补偿的启动时间
        self._yaw_k = -0.351896       # 漂移率 °/s
    ...
    def _on_timer_publish(self):
        ...
        y_deg = self.compensate_yaw(y_deg)  # 补偿漂移
    ...
    def destroy_node(self):
        ...
        try:
            ...
            pass
        if self._yaw_log_file is not None:
            self._yaw_log_file.close()
        try:
            super().destroy_node()
        except Exception:
            pass

    def _log_yaw(self, yaw_deg: float) -> None:
        """将当前时间戳（毫秒）和 yaw 角度（度）写入 CSV 文件。"""
        import os
        # 如果文件句柄未打开，则打开（追加模式）
        if self._yaw_log_file is None:
            first = not os.path.exists(self._yaw_log_path)
            self._yaw_log_file = open(self._yaw_log_path, 'a')
            if first:
                self._yaw_log_file.write('time_ms,yaw_deg\n')
        # 获取当前时间（毫秒）
        ts_ms = int(self.get_clock().now().nanoseconds / 1e6)
        self._yaw_log_file.write(f'{ts_ms},{yaw_deg:.6f}\n')
        self._yaw_log_file.flush()

    def compensate_yaw(self, raw_yaw_deg: float) -> float:
        """
        传入原始 yaw 角度（度），返回补偿后且归一化到 [-180, 180] 的角度。
        """
        if self._yaw_start_time is None:
            self._yaw_start_time = time.time()
        elapsed = time.time() - self._yaw_start_time
        compensated = raw_yaw_deg - self._yaw_k * elapsed   # self._yaw_k = -0.351896
        # 归一化到 [-180, 180]
        compensated = compensated % 360.0
        if compensated > 180.0:
            compensated -= 360.0
        return compensated
